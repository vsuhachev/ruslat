describe Ruslat::Core do
  ...
  describe ".lat_to_rus" do
    it {
      a = "Zz"
      expect { lat_to_rus(a) }.to_not(change { a })
    }
  ...
  describe ".rus_to_lat" do
    it {
      a = "Ыы"
      expect { rus_to_lat(a) }.to_not(change { a })
    }
  ...
  describe ".rus_typo_correct" do
    it {
      a = "Capa"
      expect { rus_typo_correct(a) }.to_not(change { a })
    }
  ...
  describe ".lat_typo_correct" do
    it {
      a = "Sаrа"
      expect { lat_typo_correct(a) }.to_not(change { a })
    }
